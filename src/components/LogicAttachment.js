...
export default  createComponentClass(/** @lends platypus.components.LogicAttachment.prototype */{
    ...
    initialize: function () {
        ...
        if (events) {
            ...
            for (let i = 0; i < length; i++) {
                ...
                this.addEventListener(key, (...args) => {
                    const
                        {attachment} = this;
                    
                    if (attachment) {
                        attachment.trigger.call(attachment, event, ...args);
                    }
                });
            }
        }
    ...
    getAssetList: function (def, props, defaultProps) {
        const
            attachment = def?.attachment ?? props?.attachment ?? defaultProps?.attachment;
