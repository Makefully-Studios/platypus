...
export default createComponentClass(/** @lends platypus.components.HandlerLogic.prototype */{
    id: "HandlerLogic",
    properties: {
        /**
    ...
    events: {
        "child-entity-updated": function (entity) {
            const
                j = this.entities.indexOf(entity),
        ...
        "camera-update": function (camera) {
            ...
            if (cam) {
                const
                    buffer = this.buffer === -1 ? camera.viewport.width / 10 : this.buffer, // sets a default buffer based on the size of the world units if the buffer was not explicitly set.
                    vp = camera.viewport;
        ...
        "tick": function (resp) {
            const
                {activeEntities: actives, message: msg, removals} = this,
    ...
    //        this.leftoverTime = Math.max(this.leftoverTime - (cycles * this.stepLength), 0);
            ...
            while (cycles--) {
                ...
                if (!this.paused) {
                    ...
                    if (removals.length) {
                        let j = removals.length;

                        while (j--) {
                            let k = actives.indexOf(removals[j]);

                            if (k >= 0) {
                                greenSplice(actives, k);
                            }
                        }
                        removals.length = 0;
                    }
